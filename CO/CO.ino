...
const float Sf1 = 4.05; //nA/ppm change this to match your barcode!!!!   ***SO2 = 11.61   NO2 = -22.07     CO = 4.05
...
void setup() {
  ...
  sensor1.pVcc = 5.01;  //analogRead Reference Voltage, maybe measure Aref??
  sensor1.pVsup = 3.3;  //voltage supplied to V+ of ULP, default is 3.3 Volts, probably should measure this as well.
...
void loop() {
  ...
  if (millis() - etime > (s * 1000)) {//also handles case where millis rolls over, but may cause timing issue.
    ...
    Serial.print(sensor1.convertT('F')); //use 'C' or 'F' for units
    Serial.print(", ");
